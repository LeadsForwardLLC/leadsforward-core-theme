...
/**
 * Whether the current user can use front-end editing tools (floating assistant + libraries).
 */
function lf_ai_editing_user_can(): bool {
	return current_user_can(LF_AI_CAP) || current_user_can('edit_theme_options');
}

/**
 * Service rows for the front-end service card picker (all statuses; drafts included).
 *
 * @return list<array<string, mixed>>
 */
function lf_ai_fetch_service_library_rows(string $search = ''): array {
	$query_args = [
...
/**
 * Service area rows for the front-end area card picker (all statuses; drafts included).
 *
 * @return list<array<string, mixed>>
 */
function lf_ai_fetch_area_library_rows(string $search = ''): array {
	$query_args = [
...
/**
 * Normalize POST context_id for AJAX: homepage (any case) vs numeric post ID.
 *
 * @return 'homepage'|int
 */
function lf_ai_ajax_normalize_context_id(string $context_id): string|int {
	$t = strtolower(trim($context_id));
	if ($t === 'homepage') {
		return 'homepage';
	}
...
/**
 * Remove inline DOM overrides for service details checklist item selectors after a canonical checklist save.
 *
 * @param string $ctx_type Context type (e.g. homepage, post).
 * @param mixed  $ctx_id   Context id (normalized).
 * @return int Number of override keys removed.
 */
function lf_ai_clear_checklist_overrides(string $ctx_type, $ctx_id): int {
	if (!function_exists('lf_ai_get_inline_dom_overrides') || !function_exists('lf_ai_set_inline_dom_overrides')) {
		return 0;
	}
...
function lf_ai_ajax_area_library(): void {
	...
	$search = isset($_POST['search']) ? sanitize_text_field(wp_unslash((string) $_POST['search'])) : '';
	wp_send_json_success([
		'items' => lf_ai_fetch_area_library_rows($search),
	]);
}
