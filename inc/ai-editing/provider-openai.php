...
add_filter('lf_ai_text_completion', 'lf_ai_openai_text_completion', 10, 5);
...
/**
 * Plain-text completions (inline rewrite, etc.) — no JSON response_format.
 */
function lf_ai_openai_text_completion($response, string $system, string $user, string $context_type, $context_id) {
	...
	$max_tokens = 900;
	if (function_exists('mb_strlen') && mb_strlen($user) > 2500) {
		$max_tokens = 1400;
	}
	$payload = [
		'model'       => 'gpt-4o-mini',
