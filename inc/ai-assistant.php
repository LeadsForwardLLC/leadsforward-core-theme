...
function lf_ai_assistant_assets(string $hook = ''): void {
	...
	if ($ctx_id > 0 && in_array($ctx_type, ['post', 'page', 'lf_service', 'lf_service_area', 'lf_project'], true)) {
	...
	<div class="lf-ai-inline-link" data-lf-ai-inline-link-root>
		<div class="lf-ai-inline-link__toolbar" data-lf-ai-inline-link-toolbar hidden>
			<div class="lf-ai-inline-link__toolbar-inner">
				<span class="lf-ai-inline-link__toolbar-group" data-lf-ai-heading-tools hidden role="group" aria-label="<?php esc_attr_e('Heading level', 'leadsforward-core'); ?>">
					<?php
					for ($lf_hi = 1; $lf_hi <= 6; $lf_hi++) {
						echo '<button type="button" class="lf-ai-inline-link__fmt lf-ai-inline-link__fmt--h" data-lf-ai-heading-level="' . (int) $lf_hi . '" title="' . esc_attr(sprintf(/* translators: %d: heading level 1–6 */ __('Use heading H%d', 'leadsforward-core'), $lf_hi)) . '">H' . (int) $lf_hi . '</button>';
					}
					?>
				</span>
				...
				<span class="lf-ai-inline-link__toolbar-group" role="group" aria-label="<?php esc_attr_e('Text emphasis', 'leadsforward-core'); ?>">
					...
					<button type="button" class="lf-ai-inline-link__fmt" data-lf-ai-inline-cmd="italic" title="<?php esc_attr_e('Italic', 'leadsforward-core'); ?>"><em>I</em></button>
	...
	<div class="lf-ai-float lf-ai-float--history" data-lf-ai-history-float>
		...
		<div class="lf-ai-float__panel lf-ai-float__panel--history" id="lf-ai-history-panel" hidden>
			<div class="lf-ai-float__header">
				...
				<div class="lf-ai-float__header-actions">
					<button type="button" class="lf-ai-float__icon" data-lf-ai-history-refresh aria-label="<?php esc_attr_e('Reload list from server', 'leadsforward-core'); ?>" title="<?php esc_attr_e('Reload list from server (fetches the latest restore points)', 'leadsforward-core'); ?>">↻</button>
			...
			<div class="lf-ai-float__body">
				<p class="lf-ai-history__hint"><?php esc_html_e('Each save creates a restore point (who + when). The header refresh icon reloads this list from the server; use it after a teammate saves or if restore says the layout changed. Restoring reloads the page.', 'leadsforward-core'); ?></p>
...
function lf_ai_assistant_widget_css(): string {
	return '
		...
		.lf-ai-float__icon.lf-is-loading { pointer-events:none; opacity:0.72; animation: lf-ai-spin 0.7s linear infinite; }
		@keyframes lf-ai-spin { to { transform: rotate(360deg); } }
		.lf-ai-history__badge { display:inline-block; font-size:10px; font-weight:800; text-transform:uppercase; letter-spacing:0.04em; padding:2px 8px; border-radius:999px; background:#dcfce7; color:#166534; border:1px solid #bbf7d0; margin-bottom:2px; }
		.lf-ai-history__row.is-live { border-color:#86efac; background:#f0fdf4; }
		.lf-ai-history__restore[disabled] { opacity:0.55; cursor:default; background:#94a3b8 !important; }
		...
		.lf-ai-inline-link__toolbar-inner { display:flex; flex-wrap:wrap; gap:8px; align-items:center; padding:6px 10px; border-radius:10px; background:#1e1b4b; color:#fff; box-shadow:0 8px 24px rgba(15,23,42,.35); font-size:12px; font-weight:600; max-width:min(420px,calc(100vw - 24px)); }
		.lf-ai-inline-link__toolbar-group { display:inline-flex; gap:4px; align-items:center; padding-right:8px; margin-right:2px; border-right:1px solid rgba(255,255,255,.2); }
		.lf-ai-inline-link__toolbar-group:last-of-type { border-right:0; padding-right:0; margin-right:0; }
...
function lf_ai_assistant_widget_js(): string {
	...
	return 'jQuery(function($){
		...
		function loadRevisionHistory($ctxRoot) {
			var $list = ($ctxRoot && $ctxRoot.length) ? $ctxRoot.find("[data-lf-ai-history-list]") : $historyList;
			var $status = ($ctxRoot && $ctxRoot.length) ? $ctxRoot.find("[data-lf-ai-history-status]") : $historyStatus;
			var $refBtn = ($ctxRoot && $ctxRoot.length) ? $ctxRoot.find("[data-lf-ai-history-refresh]") : $historyRefresh;
			if (!$list.length) return;
			$status.text("").removeClass("is-error");
			if ($refBtn.length) {
				$refBtn.addClass("lf-is-loading").attr("aria-busy", "true");
			}
			var refreshing = (lfAiFloating.i18n && lfAiFloating.i18n.historyRefreshing) ? lfAiFloating.i18n.historyRefreshing : "Refreshing history…";
			...
			$.post(lfAiFloating.ajax_url, {
				...
				if ($refBtn.length) {
					$refBtn.removeClass("lf-is-loading").removeAttr("aria-busy");
				}
				$status.text("").removeClass("is-error");
				if (!res || !res.success || !res.data) {
					$status.text((lfAiFloating.i18n && lfAiFloating.i18n.historyLoadFailed) ? lfAiFloating.i18n.historyLoadFailed : "Could not load history.").addClass("is-error");
					return;
				}
				if (typeof res.data.layout_version === "number") {
					layoutVersion = res.data.layout_version;
				}
				var liveVer = typeof res.data.layout_version === "number" ? res.data.layout_version : layoutVersion;
				var rows = Array.isArray(res.data.revisions) ? res.data.revisions : [];
				if (!rows.length) {
					$list.html("<p class=\"lf-ai-history__empty\" style=\"font-size:12px;color:#64748b;margin:0;\">" + escapeHtml((lfAiFloating.i18n && lfAiFloating.i18n.historyEmpty) ? lfAiFloating.i18n.historyEmpty : "No revisions yet.") + "</p>");
					return;
				}
				...
				rows.forEach(function(row){
					...
					var rowVer = parseInt(String(row && row.layout_version != null ? row.layout_version : "0"), 10) || 0;
					var isLive = rowVer > 0 && rowVer === liveVer;
					html += "<div class=\"lf-ai-history__row" + (isLive ? " is-live" : "") + "\" data-lf-rev-id=\"" + escapeHtml(id) + "\">";
					if (isLive) {
						html += "<span class=\"lf-ai-history__badge\" title=\"" + escapeHtml(liveHelp) + "\">" + escapeHtml(liveLabel) + "</span>";
					}
		...
		function lfManagedContentEditableHost() {
			...
			if (ae.matches(".lf-benefits__title, .lf-benefits__desc, .lf-service-details__text, .lf-service-details__micro-text, .lf-block-hero__card-item-text, .lf-section__intro, .lf-rich-content__prose, [data-lf-inline-editable=\"1\"]")) {
				return ae;
			}
			return null;
		}
		...
		function lfFindHeadingInHost(host) {
			if (!host) return null;
			var sel = window.getSelection();
			...
			while (el && el !== host) {
				var tn = el.tagName ? String(el.tagName).toLowerCase() : "";
				if (/^h[1-6]$/.test(tn)) return el;
				el = el.parentElement;
			}
			return null;
		}
		function lfHeadingUsesConfigurableSectionTag(hEl, wrap) {
			if (!hEl || !wrap) return false;
			if (hEl.closest(".lf-benefits__card")) return false;
			if (hEl.classList.contains("lf-benefits__title")) return false;
			if (hEl.closest(".lf-rich-content__prose")) return false;
			if (hEl.classList.contains("lf-layout-heading__text")) return true;
			if (hEl.classList.contains("lf-section__title")) return true;
			var cn = String(hEl.className || "");
			return /\blf-block-[a-z0-9-]+__title\b/.test(cn);
		}
		...
		function lfInitInlineLinkUi() {
			...
			$linkRoot.on("mousedown", "[data-lf-ai-inline-cmd]", function(e){
				e.preventDefault();
			});
			$linkRoot.on("click", "[data-lf-ai-inline-cmd]", function(e){
				e.preventDefault();
				var host = lfAnyInlineLinkHostEl();
				if (!host) return;
				try { host.focus(); } catch (errF) {}
				var cmd = String($(this).attr("data-lf-ai-inline-cmd") || "");
				var cmdVal = String($(this).attr("data-lf-ai-inline-cmd-value") || "");
				if (cmd === "foreColor" && cmdVal !== "") {
					try { document.execCommand("foreColor", false, cmdVal); } catch (errC) {}
					return;
				}
				if (cmd !== "") {
			...
			$linkRoot.on("click", "[data-lf-ai-heading-level]", function(e){
				e.preventDefault();
				var lvl = parseInt($(this).attr("data-lf-ai-heading-level") || "0", 10);
				...
				if (lfHeadingUsesConfigurableSectionTag(hEl, wrap)) {
					if (sectionType === "layout_heading") {
						var layoutTag = tag;
						if (lvl < 2 || lvl > 4) layoutTag = "h2";
						persistSectionStyle(wrap, "set_layout_heading_tag", { heading_tag: layoutTag });
					} else {
						persistSectionStyle(wrap, "set_section_heading_tag", { heading_tag: tag });
					}
					return;
				}
		...
		function setHistoryOpen(open) {
			...
			if (open) {
				$historyPanel.prop("hidden", false);
				try { $historyPanel[0].removeAttribute("hidden"); } catch (eHh) {}
			} else {
		...
		function setAiOpen(open) {
			...
			if (open) {
				...
				try { $panel[0].removeAttribute("hidden"); } catch (eH0) {}
			} else {
				$panel.prop("hidden", true);
			}
		...
		function setSeoOpen(open) {
			...
			if (open) {
				$seoPanel.prop("hidden", false);
				try { $seoPanel[0].removeAttribute("hidden"); } catch (eHs) {}
			} else {
				$seoPanel.prop("hidden", true);
			}
		...
		function inlineNodeEligible(node) {
			...
			if (node.closest(".lf-hero-chips,.lf-hero-conversion__badges,.lf-stats-bar__grid,.lf-trust-bar__badges,.lf-process,.lf-block-faq-accordion__list,.lf-related-links,.lf-cpt-driven-links")) return false;
			...
			// Rich text sections: one contenteditable host for the full prose (rich_content_body).
			if (node.closest(".lf-rich-content__prose")) return false;
		...
		function buildRichContentProseTargets() {
			var proseNodes = document.querySelectorAll("[data-lf-section-wrap=\"1\"][data-lf-section-type=\"rich_content\"] .lf-rich-content__prose");
			if (!proseNodes || !proseNodes.length) return;
			Array.prototype.forEach.call(proseNodes, function(prose){
				if (!prose || prose.closest(".lf-ai-float,.lf-ai-seo-float")) return;
				var selector = buildInlineSelector(prose);
				if (!selector) return;
				prose.setAttribute("data-lf-inline-editable", "1");
				prose.setAttribute("data-lf-inline-selector", selector);
				prose.setAttribute("data-lf-inline-field-key", "rich_content_body");
				var sectionWrap = prose.closest("[data-lf-section-wrap=\"1\"][data-lf-section-id]");
				var sectionId = sectionWrap ? String(sectionWrap.getAttribute("data-lf-section-id") || "") : "";
				if (sectionId) {
					prose.setAttribute("data-lf-inline-section-id", sectionId);
				} else {
					prose.removeAttribute("data-lf-inline-section-id");
				}
				prose.removeAttribute("data-lf-inline-source-selector");
			});
		}
		...
		function buildProofBadgeControls() {
			collectSectionWrappers().forEach(function(wrap){
				...
				var badgesBox = content.querySelector(".lf-service-details__proof-badges");
				if (!badgesBox) return;
				Array.prototype.slice.call(badgesBox.querySelectorAll(".lf-service-details__proof-badge-wrap")).forEach(function(row){
					var badge = row.querySelector(".lf-service-details__proof-badge");
					if (!badge || !row.parentNode) return;
					badge.removeAttribute("data-lf-proof-badge-wired");
					badge.removeAttribute("data-lf-ai-editing");
					badge.removeAttribute("contenteditable");
					badge.removeAttribute("title");
					badge.classList.remove("lf-service-details__proof-badge--editable");
		...
		function heroPillsFromWrap(wrap) {
			var list = wrap ? (wrap.querySelector(".lf-hero-chips") || wrap.querySelector(".lf-hero-conversion__badges")) : null;
			if (!list) return [];
			...
			return Array.prototype.slice.call(list.querySelectorAll(".lf-hero-chip, .lf-hero-conversion__badge")).map(function(chip){
				...
				var label = cleanPillText(raw);
				var icon = String(chip.getAttribute("data-lf-chip-icon") || "").trim();
				if (!label) return "";
		...
		function buildHeroPillsControls() {
			...
			collectSectionWrappers().forEach(function(wrap){
				...
				var list = wrap.querySelector(".lf-hero-chips") || wrap.querySelector(".lf-hero-conversion__badges");
				if (!list) return;
				Array.prototype.slice.call(list.querySelectorAll(".lf-hero-chip,[data-lf-hero-pill-text],.lf-hero-conversion__badge")).forEach(function(node){
					node.removeAttribute("data-lf-inline-editable");
					node.removeAttribute("data-lf-inline-selector");
				});
				Array.prototype.slice.call(list.querySelectorAll(".lf-hero-chip, .lf-hero-conversion__badge")).forEach(function(chip){
					var textNode = heroPillTextNodeFromChip(chip);
		...
		function trustStatsLinesFromGrid(grid) {
			if (!grid) return [];
			return Array.prototype.slice.call(grid.querySelectorAll(".lf-stats-bar__item")).map(function(item){
				...
				var icon = String(item.getAttribute("data-lf-stat-icon") || "star").trim() || "star";
				var variant = String(item.getAttribute("data-lf-stat-variant") || "default").trim() || "default";
				if (label === "" && value === "") return "";
				var line = value + " || " + label + " || " + icon;
				if (variant && variant !== "default") line += " || " + variant;
				return line;
			}).filter(function(line){ return line !== ""; });
		}
		function syncTrustStatsGridCols(wrap) {
			if (!wrap) return;
		...
		function persistTrustStats(wrap) {
			var grid = wrap ? wrap.querySelector(".lf-stats-bar__grid") : null;
			if (!grid) return;
			syncTrustStatsGridCols(wrap);
			persistSectionLineItems(wrap, "trust_stats_items", trustStatsLinesFromGrid(grid), "Saving stats...");
		}
		function buildTrustStatsControls() {
			collectSectionWrappers().forEach(function(wrap){
				if (!wrap || wrap.closest(".lf-ai-float")) return;
				var sectionType = String(wrap.getAttribute("data-lf-section-type") || "");
				if (sectionType !== "trust_bar") return;
				Array.prototype.slice.call(wrap.querySelectorAll("[data-lf-ai-trust-stat-controls=\"1\"],[data-lf-ai-list-remove=\"1\"]")).forEach(function(node){
					if (node && node.parentNode) node.parentNode.removeChild(node);
				});
				var grid = wrap.querySelector(".lf-stats-bar__grid");
				if (!grid) return;
				function bindStatField(el) {
					if (!el) return;
					el.removeAttribute("data-lf-inline-editable");
					el.removeAttribute("data-lf-inline-selector");
					el.classList.add("lf-ai-inline-editor-ignore");
					el.setAttribute("title", "Click to edit");
					el.onmousedown = function(e){ if (e) e.stopPropagation(); };
					el.onclick = function(e){
						if (!editingEnabled) return;
						if (e) { e.preventDefault(); e.stopPropagation(); }
						if (String(el.getAttribute("data-lf-ai-editing") || "0") === "1") return;
						el.setAttribute("data-lf-ai-original-text", String(el.textContent || "").trim());
						el.setAttribute("data-lf-ai-editing", "1");
					...
					el.onblur = function(){
						...
						el.removeAttribute("data-lf-ai-editing");
						var original = String(el.getAttribute("data-lf-ai-original-text") || "");
						el.removeAttribute("data-lf-ai-original-text");
						var next = String(el.textContent || "").trim();
						if (next !== original) persistTrustStats(wrap);
					};
				}
				Array.prototype.slice.call(grid.querySelectorAll(".lf-stats-bar__item")).forEach(function(item){
					bindStatField(item.querySelector(".lf-stats-bar__value"));
					bindStatField(item.querySelector(".lf-stats-bar__label"));
					var btn = createGenericRemoveButton(function(){
						if (item && item.parentNode) item.parentNode.removeChild(item);
						persistTrustStats(wrap);
					});
					item.appendChild(btn);
				});
				var controls = document.createElement("div");
				controls.className = "lf-ai-hero-pills-controls lf-ai-inline-editor-ignore";
				controls.setAttribute("data-lf-ai-trust-stat-controls", "1");
				var addBtn = document.createElement("button");
				addBtn.type = "button";
				addBtn.className = "lf-ai-hero-pill-add lf-ai-inline-editor-ignore";
				...
				addBtn.addEventListener("click", function(e){
					...
					var label = "";
					var icon = "star";
					try { value = String(window.prompt("Stat value (e.g. 10+):", "") || "").trim(); } catch (err) { value = ""; }
					if (!value) return;
					try { label = String(window.prompt("Stat label (e.g. Years in Business):", "") || "").trim(); } catch (err2) { label = ""; }
					if (!label) return;
					try { icon = String(window.prompt("Icon slug (optional, e.g. hammer):", "star") || "star").trim() || "star"; } catch (err3) { icon = "star"; }
					var item = document.createElement("div");
					item.className = "lf-stats-bar__item lf-stats-bar__item--default";
					item.setAttribute("role", "listitem");
					item.setAttribute("data-lf-stat-icon", icon);
					item.setAttribute("data-lf-stat-variant", "default");
					var iconWrap = document.createElement("div");
					iconWrap.className = "lf-stats-bar__icon-wrap";
					iconWrap.setAttribute("aria-hidden", "true");
					var iconSpan = document.createElement("span");
					iconSpan.className = "lf-stats-bar__icon";
					iconWrap.appendChild(iconSpan);
					var body = document.createElement("div");
					body.className = "lf-stats-bar__body";
					var valueEl = document.createElement("span");
					valueEl.className = "lf-stats-bar__value";
					valueEl.textContent = value;
					var labelEl = document.createElement("span");
					labelEl.className = "lf-stats-bar__label";
					labelEl.textContent = label;
					body.appendChild(valueEl);
					body.appendChild(labelEl);
					item.appendChild(iconWrap);
					item.appendChild(body);
					item.appendChild(createGenericRemoveButton(function(){
						if (item && item.parentNode) item.parentNode.removeChild(item);
						persistTrustStats(wrap);
					}));
					grid.appendChild(item);
					bindStatField(valueEl);
					bindStatField(labelEl);
					persistTrustStats(wrap);
				});
				controls.appendChild(addBtn);
				if (grid.parentNode) {
		...
		function openSectionCtaPicker(wrap, buttonNode, isBenefits) {
			...
			if (isBenefits) {
				...
				if (textLabCopy) textLabCopy.textContent = "Button text";
		...
		function persistSectionStyle(wrap, patch, extra) {
			...
			if (extra && typeof extra === "object") {
				...
				if (extra.heading_tag) payload.heading_tag = String(extra.heading_tag);
			}
			$.post(lfAiFloating.ajax_url, payload).done(function(res){
				if (res && res.success) {
					setStatus((res.data && res.data.message) ? res.data.message : "Saved.", false);
		...
		function persistRichContentBodyNow(proseEl, successMessage, done) {
			if (!proseEl) {
				if (typeof done === "function") done(false);
				return;
			}
			var fieldKey = String(proseEl.getAttribute("data-lf-inline-field-key") || "");
			var sectionId = String(proseEl.getAttribute("data-lf-inline-section-id") || "");
			var selector = String(proseEl.getAttribute("data-lf-inline-selector") || "");
			if (fieldKey !== "rich_content_body" || !sectionId) {
				setStatus("Could not save rich text (missing section binding).", true);
				if (typeof done === "function") done(false);
				return;
			}
			var valueHtml = String(proseEl.innerHTML || "").trim();
			var valueText = String(proseEl.textContent || "").trim();
			if (valueText === "") {
				setStatus("Text cannot be empty.", true);
				if (typeof done === "function") done(false);
				return;
			}
			var wrap = proseEl.closest("[data-lf-section-wrap=\"1\"][data-lf-section-id]");
			var ctx = persistContextFromWrap(wrap);
			setStatus("Saving rich text...", false);
			$.post(lfAiFloating.ajax_url, {
				action: "lf_ai_inline_save",
				nonce: lfAiFloating.nonce,
				context_type: ctx.context_type,
		...
		function showShortcutHelp() {
			var lines = [
				...
				"Layout History: header refresh icon reloads from server"
			];
			var msg = lines.join("\n");
		...
		$(document).on("click", "[data-lf-ai-toggle]", function(e){
			e.preventDefault();
			var $float = $(this).closest("[data-lf-ai-float]");
			if (!$float.length || !$root.length || $float[0] !== $root[0]) return;
		...
		$(document).on("click", "[data-lf-ai-seo-toggle]", function(e){
			e.preventDefault();
			var $float = $(this).closest("[data-lf-ai-seo-float]");
			if (!$seoRoot.length || !$float.length || $float[0] !== $seoRoot[0]) return;
		...
		$(document).on("click", "[data-lf-ai-history-toggle]", function(e){
			e.preventDefault();
			var $float = $(this).closest("[data-lf-ai-history-float]");
			if (!$historyRoot.length || !$float.length || $float[0] !== $historyRoot[0]) return;
		...
		$(document).on("click", "[data-lf-ai-history-refresh]", function(e){
			...
			if (!$ctx.length || ($historyRoot.length && $ctx[0] !== $historyRoot[0])) return;
			loadRevisionHistory($ctx);
		});
		$(document).on("click", "[data-lf-rev-restore]", function(e){
			e.preventDefault();
			var $btn = $(this);
			if ($btn.prop("disabled")) return;
			var $ctx = $btn.closest("[data-lf-ai-history-float]");
			if (!$ctx.length) return;
			var $hStatus = $ctx.find("[data-lf-ai-history-status]");
			var rid = String($btn.attr("data-lf-rev-restore") || "");
			if (!rid) return;
			...
			if (!ok) return;
			$hStatus.text("").removeClass("is-error");
			$.post(lfAiFloating.ajax_url, {
				...
				$.post(lfAiFloating.ajax_url, {
					...
					if (res && res.success) {
						...
						$hStatus.text((res && res.data && res.data.message) ? res.data.message : "Restore failed.").addClass("is-error");
					}
				}).fail(function(xhr){
					var m = (xhr.responseJSON && xhr.responseJSON.data && xhr.responseJSON.data.message) ? xhr.responseJSON.data.message : "Restore failed.";
					$hStatus.text(m).addClass("is-error");
				});
			}).fail(function(){
				$hStatus.text((lfAiFloating.i18n && lfAiFloating.i18n.historyLoadFailed) ? lfAiFloating.i18n.historyLoadFailed : "Could not verify version.").addClass("is-error");
			});
		});
