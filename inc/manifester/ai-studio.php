...
/**
 * Admin URL for Site Setup (Airtable sync + template build).
 *
 * @param array<string, scalar|null> $args Query arguments for add_query_arg().
 */
function lf_ai_studio_site_setup_admin_url(array $args = []): string {
	$slug = defined('LF_SITE_SETUP_ADMIN_SLUG') ? LF_SITE_SETUP_ADMIN_SLUG : 'lf-site-setup';
	$url = admin_url('admin.php?page=' . $slug);
	return $args === [] ? $url : add_query_arg($args, $url);
}

function lf_ai_studio_render_site_setup_page(): void {
	lf_ai_studio_render_admin_page('site_setup');
}

function lf_ai_studio_render_page(): void {
	lf_ai_studio_render_admin_page('manifester');
}
...
/**
 * Parse Sitemap Sync option (lf_airtable_sitemap_cache) for smoke-test pickers.
 * Same path rules as the Manifest page: /services/* and /service-areas/*.
 *
 * @return array{service_slugs: array<string, string>, area_slugs: array<string, string>}
 */
function lf_ai_studio_smoke_test_slugs_from_sitemap_cache(?string $cache_raw = null): array {
	$service_slugs = [];
	$area_slugs = [];
	$raw = $cache_raw !== null ? $cache_raw : (string) get_option('lf_airtable_sitemap_cache', '');
	$cache = $raw !== '' ? json_decode($raw, true) : null;
	if (!is_array($cache)) {
		return ['service_slugs' => $service_slugs, 'area_slugs' => $area_slugs];
	}
	foreach ($cache as $spec) {
		...
		if ($group === 'Services') {
			if (strpos($slug_for_check, '/services/') === 0 && $slug_for_check !== '/services/') {
				...
				if ($key !== '') {
					$service_slugs[$key] = $title !== '' ? $title : $key;
				}
			}
		}
		if ($group === 'Service Areas') {
			if (strpos($slug_for_check, '/service-areas/') === 0 && $slug_for_check !== '/service-areas/') {
				$key = sanitize_title((string) basename(trim($slug_for_check, '/')));
				if ($key !== '') {
					$area_slugs[$key] = $title !== '' ? $title : $key;
				}
			}
		}
	}
...
function lf_ai_studio_assets(string $hook): void {
	...
	$site_setup_hook = 'leadsforward_page_' . (defined('LF_SITE_SETUP_ADMIN_SLUG') ? LF_SITE_SETUP_ADMIN_SLUG : 'lf-site-setup');
	if ($hook !== $manifest_hook && $hook !== $site_setup_hook) {
		return;
	}
	wp_enqueue_media();
	wp_enqueue_style(
	...
	wp_enqueue_script(
		'lf-ai-studio-airtable',
		(defined('LF_MANIFESTER_URI') ? LF_MANIFESTER_URI : LF_THEME_URI . '/inc/manifester') . '/assets/js/ai-studio-airtable.js',
		[],
		LF_THEME_VERSION,
		true
	);
	...
	wp_localize_script('lf-ai-studio-airtable', 'LFAirtableManifester', [
		...
		'manifestSaved' => function_exists('lf_ai_studio_manifest_exists') && lf_ai_studio_manifest_exists(),
		'storedProjectRecordId' => trim((string) get_option('lf_ai_airtable_project_record_id', '')),
		'strings' => [
			...
			'scopeNoTargets' => __('Enable at least one target in Site Setup and click Save scope before orchestrator runs.', 'leadsforward-core'),
...
function lf_ai_studio_handle_scope_save(): void {
	...
	// Slug mode only. Keep legacy ID options cleared so generation cannot silently read an old mode.
	update_option('lf_ai_scope_service_ids', [], false);
	update_option('lf_ai_scope_service_area_ids', [], false);
	...
	wp_safe_redirect(lf_ai_studio_site_setup_admin_url(['scope_saved' => '1']));
	exit;
}
...
function lf_ai_studio_handle_save_logo(): void {
	...
	$redirect = lf_ai_studio_site_setup_admin_url(['logo' => '1']);
	wp_safe_redirect($redirect);
	exit;
}
...
function lf_ai_studio_handle_images_upload(): void {
	...
	if (empty($_FILES['lf_manifest_images']) || !is_array($_FILES['lf_manifest_images'])) {
		...
		wp_safe_redirect(lf_ai_studio_site_setup_admin_url(['images_error' => 'missing']));
		exit;
	}
	$result = lf_ai_studio_process_images_upload($_FILES['lf_manifest_images']);
	$user_id = get_current_user_id();
	...
	$redirect = lf_ai_studio_site_setup_admin_url(['images_uploaded' => (string) $result['uploaded_count']]);
...
function lf_ai_studio_render_admin_page(string $mode = 'manifester'): void {
	if (!current_user_can('edit_theme_options')) {
		return;
	}
	$mode = $mode === 'site_setup' ? 'site_setup' : 'manifester';
	$is_site_setup = $mode === 'site_setup';
	$site_setup_url = lf_ai_studio_site_setup_admin_url();
	$manifester_url = lf_ai_studio_manifest_admin_url();
	$import_content_url = admin_url('admin.php?page=lf-import-page-content');
	...
	$cache_split = lf_ai_studio_smoke_test_slugs_from_sitemap_cache($cache_raw);
	$cache_service_slugs = $cache_split['service_slugs'];
	$cache_area_slugs = $cache_split['area_slugs'];
	...
	?>
	<div class="wrap">
		<?php if ($is_site_setup) : ?>
			<h1 class="lf-manifester-page-title"><?php esc_html_e('Site Setup', 'leadsforward-core'); ?></h1>
			<p class="lf-manifester-lead description"><?php esc_html_e('Pull project data from Airtable, scaffold pages and niche sections, then import finished copy with page templates.', 'leadsforward-core'); ?></p>
		<?php else : ?>
			<h1 class="lf-manifester-page-title"><?php esc_html_e('AI Manifester (n8n)', 'leadsforward-core'); ?></h1>
			<p class="lf-manifester-lead description"><?php esc_html_e('Optional full-site AI generation via n8n. Complete Site Setup first, then use Import Page Content for writer-driven copy.', 'leadsforward-core'); ?></p>
		<?php endif; ?>
			...
			<div class="lf-manifester-hero__main">
				...
				<?php if ($is_site_setup) : ?>
					<ol class="lf-manifester-hero__steps">
						<li><?php esc_html_e('Add Airtable credentials in Global Settings.', 'leadsforward-core'); ?></li>
						<li><?php esc_html_e('Select an Airtable project and save build scope.', 'leadsforward-core'); ?></li>
						<li><?php esc_html_e('Build site (templates) — scaffolds pages, sections, and niche images.', 'leadsforward-core'); ?></li>
						<li><?php esc_html_e('Import page copy via Import Page Content.', 'leadsforward-core'); ?></li>
					</ol>
				<?php else : ?>
					<ol class="lf-manifester-hero__steps">
						<li><?php esc_html_e('Complete Site Setup (Airtable sync + template build).', 'leadsforward-core'); ?></li>
						<li><?php esc_html_e('Connect n8n webhook + secret in Global Settings.', 'leadsforward-core'); ?></li>
						<li><?php esc_html_e('Run Generate (orchestrator) for full AI-written copy.', 'leadsforward-core'); ?></li>
					</ol>
				<?php endif; ?>
				...
				<?php elseif ($airtable_ready) : ?>
					<p class="lf-manifester-hero__status">
						<?php echo $is_site_setup
							? esc_html__('Airtable connected — pick a project below.', 'leadsforward-core')
							: esc_html__('Airtable connected — complete Site Setup if you have not yet.', 'leadsforward-core'); ?>
					</p>
				<?php else : ?>
				...
				<?php endif; ?>
				<?php if (!$is_site_setup && !$manifest_exists) : ?>
					<p class="description" style="margin-top:8px;">
						<a class="button button-secondary" href="<?php echo esc_url($site_setup_url); ?>"><?php esc_html_e('Open Site Setup', 'leadsforward-core'); ?></a>
					</p>
				<?php endif; ?>
			</div>
		</div>
			...
			<div class="lf-manifester-steps">
				...
				<div class="lf-manifester-step">
					...
					<div class="lf-manifester-step__content">
						...
						<p class="description"><?php esc_html_e('Airtable PAT, base ID, and projects table. n8n webhook is only needed for the optional AI path.', 'leadsforward-core'); ?></p>
				...
				<div class="lf-manifester-step">
					<div class="lf-manifester-step__badge">1</div>
					<div class="lf-manifester-step__content">
						<h3><?php esc_html_e('Site Setup (required first)', 'leadsforward-core'); ?></h3>
						<p class="description"><?php esc_html_e('Sync Airtable data and run template build before orchestrator generation.', 'leadsforward-core'); ?></p>
						<p class="description lf-manifester-step__scope">
							<strong><?php esc_html_e('Current scope:', 'leadsforward-core'); ?></strong>
							<?php
							if (!empty($scope_snap['enabled_labels'])) {
								echo esc_html(implode(', ', $scope_snap['enabled_labels']));
							} else {
								esc_html_e('Not set — configure in Site Setup.', 'leadsforward-core');
							}
							?>
						</p>
						<a class="button button-primary" href="<?php echo esc_url($site_setup_url); ?>"><?php esc_html_e('Open Site Setup', 'leadsforward-core'); ?></a>
					</div>
				</div>
				<?php endif; ?>
					...
					<div class="lf-manifester-step__content">
						...
						<div class="lf-manifester-source">
							<div class="lf-manifester-panel" id="lf-airtable-picker">
								...
								$last_errors = is_array($last_audit['errors'] ?? null) ? array_values(array_filter(array_map('strval', $last_audit['errors']))) : [];
								$last_counts = is_array($last_audit['counts'] ?? null) ? $last_audit['counts'] : [];
								$last_ts = isset($last_audit['timestamp']) ? (int) $last_audit['timestamp'] : 0;
								$last_record = trim((string) get_option('lf_ai_airtable_project_record_id', ''));
								?>
								<?php if (!empty($last_audit)) : ?>
									<details class="lf-scope-audit">
										<summary><?php esc_html_e('Last payload audit', 'leadsforward-core'); ?></summary>
										<p class="description" style="margin:8px 0 0;">
											<?php
											$summary_parts = [];
											if ($last_record !== '') {
												$summary_parts[] = sprintf(__('record %s', 'leadsforward-core'), $last_record);
											}
											if ($last_ts > 0) {
												$summary_parts[] = wp_date('Y-m-d H:i', $last_ts);
											}
											$total = isset($last_counts['blueprints_total']) ? (int) $last_counts['blueprints_total'] : 0;
											if ($total > 0) {
												$summary_parts[] = sprintf(__('%d blueprints', 'leadsforward-core'), $total);
											}
											echo esc_html(implode(' · ', $summary_parts));
											if (!empty($last_errors)) {
												echo ' — ' . esc_html($last_errors[0]);
											}
											?>
										</p>
									</details>
								...
								<div class="lf-airtable-grid">
									...
									<div class="lf-airtable-preview">
										<div id="lf-airtable-preview" class="lf-airtable-preview-card">
											<?php esc_html_e('Select a project to preview.', 'leadsforward-core'); ?>
										</div>
										<div id="lf-airtable-status" class="lf-airtable-status" role="status" aria-live="polite"></div>
									</div>
								</div>
								<form id="lf-scope-form" class="lf-scope-panel" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
				...
				<div class="lf-manifester-step">
					...
					<div class="lf-manifester-step__content">
						...
						<p class="description">
							<?php esc_html_e('Each page template includes a curated niche photo pack. Template build slots those images into hero and content areas automatically.', 'leadsforward-core'); ?>
						</p>
				...
				<div class="lf-manifester-step lf-manifester-step--action">
					...
					<div class="lf-manifester-step__content">
						<h3><?php esc_html_e('Build site (templates)', 'leadsforward-core'); ?></h3>
						<p class="description"><?php esc_html_e('Scaffolds pages, menus, niche section order, and placeholder images from your Airtable manifest — no n8n required.', 'leadsforward-core'); ?></p>
						<p class="description lf-manifester-step__scope">
						...
						<p class="description lf-manifester-step__note"><?php esc_html_e('After build, paste Google Doc copy in Import Page Content.', 'leadsforward-core'); ?></p>
				...
				<div class="lf-manifester-step">
					...
					<div class="lf-manifester-step__content">
						<h3><?php esc_html_e('Import page copy', 'leadsforward-core'); ?></h3>
						<p class="description"><?php esc_html_e('Writers paste formatted docs to populate homepage and core pages.', 'leadsforward-core'); ?></p>
						<a class="button button-primary" href="<?php echo esc_url($import_content_url); ?>"><?php esc_html_e('Open Import Page Content', 'leadsforward-core'); ?></a>
						<p class="description lf-manifester-step__note" style="margin-top:8px;">
							<?php esc_html_e('Optional: use AI Manifester (n8n) instead of manual docs for full-site AI copy.', 'leadsforward-core'); ?>
							<a href="<?php echo esc_url($manifester_url); ?>"><?php esc_html_e('Open AI Manifester', 'leadsforward-core'); ?></a>
						</p>
					</div>
				</div>
				<?php endif; ?>
				...
				<div class="lf-manifester-step lf-manifester-step--action">
					<div class="lf-manifester-step__badge">3</div>
					<div class="lf-manifester-step__content">
						<h3><?php esc_html_e('Generate (orchestrator)', 'leadsforward-core'); ?></h3>
						<p class="description"><?php esc_html_e('Runs the full n8n AI pipeline using the manifest and scope from Site Setup.', 'leadsforward-core'); ?></p>
						<p class="description lf-manifester-step__scope">
							<strong><?php esc_html_e('Scope:', 'leadsforward-core'); ?></strong>
							<?php
							if (!empty($scope_snap['enabled_labels'])) {
								...
								esc_html_e('Nothing selected — set scope in Site Setup.', 'leadsforward-core');
							}
							?>
						</p>
						<div class="lf-manifester-build-actions">
						...
						<p class="description lf-manifester-step__note"><?php esc_html_e('Requires n8n webhook + secret in Global Settings. For writer-driven copy, use Import Page Content instead.', 'leadsforward-core'); ?></p>
						<div id="lf-manifester-status" class="lf-manifester-status" role="status" aria-live="polite"></div>
						<?php
...
function lf_ai_studio_build_full_site_payload(bool $respect_manifest_scope = true): array {
	...
	// Payload audit (server-side): fail loudly before sending a broken job to n8n.
	$audit = lf_ai_studio_payload_audit_summary($blueprints, $scope);
	update_option('lf_ai_studio_last_payload_audit', $audit, false);
	if (!empty($audit['errors'])) {
		return lf_ai_studio_fail_result('build_full_site_payload', (string) ($audit['errors'][0] ?? __('Payload audit failed.', 'leadsforward-core')));
	}
 ...
 * @return array<string,mixed>
 */
function lf_ai_studio_payload_audit_summary(array $blueprints, array $scope): array {
	$errors = [];
	$count_service = 0;
	$count_service_area = 0;
	$count_wrong_service_post_type = 0;
	$count_wrong_service_area_post_type = 0;
	...
	foreach ($blueprints as $bp) {
		...
		if ($page === 'service') {
			$count_service++;
			if ($post_type !== '' && $post_type !== 'lf_service') {
				$count_wrong_service_post_type++;
			}
		}
		if ($page === 'service_area') {
			$count_service_area++;
			if ($post_type !== '' && $post_type !== 'lf_service_area') {
				$count_wrong_service_area_post_type++;
			}
		}
	}

	if (!empty($scope['services']) && $count_service === 0) {
		$errors[] = __('Payload audit failed: “Service pages” is enabled but no service blueprints were built.', 'leadsforward-core');
	}
	if (!empty($scope['service_areas']) && $count_service_area === 0) {
		$errors[] = __('Payload audit failed: “Service area pages” is enabled but no service area blueprints were built.', 'leadsforward-core');
	}
	if ($count_wrong_service_post_type > 0) {
		$errors[] = __('Payload audit failed: one or more service blueprints target the wrong post type (must be lf_service).', 'leadsforward-core');
	}
	if ($count_wrong_service_area_post_type > 0) {
		$errors[] = __('Payload audit failed: one or more service area blueprints target the wrong post type (must be lf_service_area).', 'leadsforward-core');
	}
	...
	return [
		...
		'timestamp' => time(),
		'scope' => [
			'services' => !empty($scope['services']),
			'service_areas' => !empty($scope['service_areas']),
		],
		'counts' => [
			'blueprints_total' => is_array($blueprints) ? count($blueprints) : 0,
			'service_blueprints' => $count_service,
			'service_area_blueprints' => $count_service_area,
			'wrong_service_post_type' => $count_wrong_service_post_type,
			'wrong_service_area_post_type' => $count_wrong_service_area_post_type,
		],
		'errors' => $errors,
	];
}
...
function lf_ai_studio_deduplicate_headings(array $items, array $registry, bool $is_page_builder = false): array {
	...
	foreach ($items as $id => $row) {
		...
		if (!$is_page_builder && function_exists('lf_homepage_base_section_type')) {
			$base_t = lf_homepage_base_section_type((string) $id);
			if ($base_t !== '' && isset($registry[$base_t])) {
				$type = $base_t;
			}
		}
		$label = '';
...
/**
 * Orchestrator payloads use canonical section types (hero, trust_bar). Stored homepage config keys
 * may be instance/legacy ids (hero_1, trust_reviews). These groups share one physical row in options.
 *
 * @return string[]
 */
function lf_ai_studio_homepage_equiv_storage_types(string $llm_registry_type): array {
	$t = trim($llm_registry_type);
	if ($t === '') {
		return [];
	}
	static $groups = [
		['trust_bar', 'trust_reviews'],
		['benefits', 'service_grid'],
		['map_nap', 'service_areas'],
	];
	foreach ($groups as $g) {
	...
	if (!empty($homepage_updates) && function_exists('lf_get_homepage_section_config')) {
		...
		foreach ($homepage_updates as $update) {
			...
			foreach ($fields as $key => $value) {
				...
				$llm_registry_type = function_exists('lf_homepage_base_section_type')
					? lf_homepage_base_section_type($section_id)
					: $section_id;
				...
				$allowed = lf_ai_studio_homepage_allowed_field_keys($llm_registry_type, $registry[$llm_registry_type]);
				if (!in_array($field_key, $allowed, true)) {
					if (defined('WP_DEBUG') && WP_DEBUG) {
				...
				if ($llm_registry_type === 'faq_accordion' && $field_key === 'faq_selected_ids') {
					$normalized_value = lf_ai_studio_normalize_faq_selected_ids_value($normalized_value);
				}
				$homepage_fields[$storage_key][$field_key] = $normalized_value;
		...
		foreach ($homepage_fields as $storage_key => $fields) {
			...
			$fields = lf_ai_studio_maybe_inject_section_images(
				$registry,
				$registry_type,
				is_array($config[$storage_key]) ? $config[$storage_key] : [],
				$fields,
				$homepage_matches,
		...
		foreach ($config as $section_id => $section_settings) {
			$reg_type = function_exists('lf_homepage_base_section_type')
				...
				: (string) $section_id;
			if (!is_array($section_settings) || $reg_type === '' || !isset($registry[$reg_type])) {
				continue;
			}
