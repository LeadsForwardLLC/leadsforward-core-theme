	...
	$sections = [
		'hero' => [
			...
			'fields' => [
				...
				['key' => 'hero_cta_primary_tone', 'label' => __('Primary button color', 'leadsforward-core'), 'type' => 'select', 'default' => 'secondary', 'options' => lf_sections_button_tone_options()],
				...
				['key' => 'hero_cta_secondary_tone', 'label' => __('Secondary button color', 'leadsforward-core'), 'type' => 'select', 'default' => 'secondary', 'options' => lf_sections_button_tone_options()],
		...
		'layout_button' => [
			...
			'fields' => [
				...
				['key' => 'cta_primary_tone', 'label' => __('Button color', 'leadsforward-core'), 'type' => 'select', 'default' => 'secondary', 'options' => lf_sections_button_tone_options()],
		...
		'cta' => [
			...
			'fields' => [
				...
				['key' => 'cta_primary_tone', 'label' => __('Primary button color', 'leadsforward-core'), 'type' => 'select', 'default' => 'secondary', 'options' => lf_sections_button_tone_options()],
		...
		'service_intro' => [
			...
			'fields' => [
				...
				['key' => 'service_intro_view_all_label', 'label' => __('View all services button (optional)', 'leadsforward-core'), 'type' => 'text', 'default' => ''],
		...
		'service_areas' => [
			...
			'fields' => [
				...
				['key' => 'map_intro', 'label' => __('Map intro', 'leadsforward-core'), 'type' => 'textarea', 'default' => __('When a map iframe is saved under Global Settings → Business entity, it appears here as your coverage map. Otherwise, pins plot service areas that have coordinates.', 'leadsforward-core')],
				...
				['key' => 'filter_label', 'label' => __('Filter label', 'leadsforward-core'), 'type' => 'text', 'default' => __('Filter by service area', 'leadsforward-core')],
				...
				['key' => 'service_areas_area_ids', 'label' => __('Selected service areas (IDs)', 'leadsforward-core'), 'type' => 'list', 'default' => ''],
			],
...
function lf_sections_resolve_trust_stats_items(array $settings): array {
	...
	if ($raw !== '') {
		return lf_sections_parse_trust_stats_items($raw);
	}
...
function lf_resolve_cta(array $context = [], array $section_instance = [], array $fallbacks = []): array {
	...
	if ($resolved['secondary_action'] === 'call') {
		$phone = function_exists('lf_business_entity_get')
			? trim((string) (lf_business_entity_get()['phone_display'] ?? ''))
			: '';
		if ($phone !== '') {
			$resolved['secondary_text'] = $phone;
		}
	}
	if ($resolved['primary_action'] === 'call' || $resolved['primary_type'] === 'call') {
		$phone = function_exists('lf_business_entity_get')
			? trim((string) (lf_business_entity_get()['phone_display'] ?? ''))
			: '';
		if ($phone !== '') {
			$resolved['primary_text'] = $phone;
		}
	}
...
function lf_sections_hero_cta_button_classes(array $section, string $slot, string $extra_class = ''): string {
	...
	list($style, $tone) = lf_sections_resolve_button_style_tone($style_in, $tone_in, $default_style, 'secondary');
...
function lf_sections_hero_cta_data_attrs(array $section, string $slot): string {
	...
	$tone_in = array_key_exists($t_key, $section) ? (string) $section[ $t_key ] : '';
	$default_style = $slot === 'secondary' ? 'outline' : 'solid';
	list($st, $tn) = lf_sections_resolve_button_style_tone($style_in, $tone_in, $default_style, 'secondary');
	return sprintf(
...
function lf_sections_render_service_intro(string $context, array $settings, \WP_Post $post): void {
	...
	$section = [
		...
		'service_intro_view_all_label' => $settings['service_intro_view_all_label'] ?? '',
		'section_header_align' => $settings['section_header_align'] ?? 'center',
		'icon_enabled' => $settings['icon_enabled'] ?? '0',
		'icon_slug' => $settings['icon_slug'] ?? '',
		'icon_position' => $settings['icon_position'] ?? 'left',
...
function lf_sections_render_service_areas(string $context, array $settings, \WP_Post $post): void {
	...
	$section = [
		...
		'section_background_custom' => $settings['section_background_custom'] ?? '',
		'section_header_align' => $settings['section_header_align'] ?? 'center',
		'service_areas_area_ids' => $settings['service_areas_area_ids'] ?? '',
		'icon_enabled' => $settings['icon_enabled'] ?? '0',
		'icon_slug' => $settings['icon_slug'] ?? '',
		'icon_position' => $settings['icon_position'] ?? 'left',
		'icon_size' => $settings['icon_size'] ?? 'md',
