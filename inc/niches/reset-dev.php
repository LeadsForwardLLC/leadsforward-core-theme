...
add_action('admin_menu', 'lf_dev_reset_add_menu');
...
function lf_dev_reset_handle_post(): void {
	...
	if (!isset($_POST['lf_dev_reset_confirm']) || trim($_POST['lf_dev_reset_confirm']) !== 'RESET') {
		wp_safe_redirect(admin_url('themes.php?page=lf-dev-reset&error=confirm'));
		exit;
	}
	check_admin_referer('lf_dev_reset', 'lf_dev_reset_nonce');

	lf_dev_reset_run();
	wp_safe_redirect(admin_url('themes.php?page=lf-dev-reset&reset=1'));
	exit;
}

function lf_dev_reset_add_menu(): void {
...
function lf_dev_reset_render_page(): void {
	...
	if ($done) {
		...
		echo '<p><a href="' . esc_url(admin_url('themes.php?page=lf-setup-wizard')) . '" class="button button-primary">' . esc_html__('Run setup wizard', 'leadsforward-core') . '</a></p></div>';
		return;
	}
	if ($error === 'confirm') {
	...
	echo '<form method="post" action="' . esc_url(admin_url('themes.php?page=lf-dev-reset')) . '">';
...
function lf_dev_reset_run(): void {
	...
	if (function_exists('update_field')) {
		update_field('lf_business_name', '', 'option');
		update_field('lf_business_phone', '', 'option');
		update_field('lf_business_email', '', 'option');
		update_field('lf_business_address', '', 'option');
		update_field('lf_business_hours', '', 'option');
